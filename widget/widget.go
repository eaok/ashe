...
func (w *Widget) Spawn() error {
	...
	// Add page zero reaction buttons
	w.SetPageReactions(w.Embed)
	...
	for {
		...
		if w.Timeout != 0 {
			select {
			case k := <-nextMessageReactionAddC(w):
				reaction = k.MessageReaction
			// case <-time.After(startTime.Add(w.Timeout).Sub(time.Now())):
			case <-time.After(time.Until(startTime.Add(w.Timeout))):
				return nil
			case <-w.Close:
				return nil
			}
...
// SetPageReactions sets the reactions of each embed
func (w *Widget) SetPageReactions(embed *discordgo.MessageEmbed) {
	// Add page zero reaction buttons
	if strings.HasPrefix(embed.Description, "**RED STAR QUEUE [0/4]**") {
		println("add reaction button zero")
		w.Ses.MessageReactionsRemoveAll(w.Message.ChannelID, w.Message.ID)
		w.Ses.MessageReactionAdd(w.Message.ChannelID, w.Message.ID, EmojiCheckMark)
	} else {
		println("add reaction button all")
